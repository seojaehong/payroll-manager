...
0. **기능 구현 전 필수 확인 (MUST)**
   - 이미 만들어진 기능이 아닌가? (코드베이스 검색)
   - 비슷한 기능이 다른 곳에 있지 않은가?
   - 이전에 논의/구현된 적 있는 기능인가? (히스토리 확인)
   - 새로 만드는 것보다 기존 기능 수정이 나은가?

1. **기능 충돌 검사**
   - 새 기능이 기존 기능을 깨뜨리지 않는가?
   - 중복된 코드나 함수가 없는가?
   - 같은 데이터를 다른 방식으로 처리하는 곳이 없는가?
   - 같은 URL/라우트가 두 개 이상 없는가?
